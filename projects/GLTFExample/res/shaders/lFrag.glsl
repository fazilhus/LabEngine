...
vec3 CalcPointLight(PointLight light, vec3 norm, vec3 cam_dir, vec3 frag_pos, vec3 diff, float spec);
...
void main()
{
	...
	{
		tempCol += CalcPointLight(plights[i], norm, cam_dir, pos, diff, spec);
	}
